...
test('calculatePriceModifier returns correct multipliers for different reputation levels', (t) => {
  ...
  t.is(ReputationManager.calculatePriceModifier(-10), 1)
  t.is(ReputationManager.calculatePriceModifier(0), 1)
  t.is(ReputationManager.calculatePriceModifier(15), 1)
...
test('getReputationModifier returns complete modifier objects', (t) => {
  ...
  t.is(neutralModifier.priceMultiplier, 1)
...
test('getLocationReputation combines global and location reputation correctly', (t) => {
  ...
  const locationRep = ReputationManager.getLocationReputation(
    reputation,
    'Market Square'
  )
  t.is(locationRep, 32)
  ...
  // 0 * 0.6 + 20 * 0.4 = 0 + 8 = 8
  const unknownLocationRep = ReputationManager.getLocationReputation(
    reputation,
    'Unknown Location'
  )
...
test('getNPCReputation combines location and NPC reputation correctly', (t) => {
  ...
  const npcRep = ReputationManager.getNPCReputation(
    reputation,
    'merchant_aldric',
    'Market Square'
  )
  ...
  const unknownNPCRep = ReputationManager.getNPCReputation(
    reputation,
    'unknown_npc',
    'Market Square'
  )
...
test('calculateReputationLoss returns negative values based on severity', (t) => {
  ...
  for (let severity = 1; severity <= 5; severity++) {
    ...
    t.true(
      loss >= -(severity * 2 + severity),
      `Loss should be within expected range for severity ${severity}`
    )
    t.true(
      loss <= -(severity * 2),
      `Loss should be at least base amount for severity ${severity}`
    )
  }
