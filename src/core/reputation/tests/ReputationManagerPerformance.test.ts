...
test('ReputationManager price modifier caching improves performance', t => {
  ...
  const modifiers1 = reputationValues.map(rep => ReputationManager.calculatePriceModifier(rep))
  const time1 = performance.now() - start1
  
  // Second pass - use cache
  const start2 = performance.now()
  const modifiers2 = reputationValues.map(rep => ReputationManager.calculatePriceModifier(rep))
  ...
  t.true(time2 < time1, `Cached calls (${time2}ms) should be faster than initial calls (${time1}ms)`)
  
  // Verify correct values
  t.is(modifiers1[0], 1.5) // Despised
  t.is(modifiers1[2], 1.0) // Neutral
...
test('ReputationManager reputation level caching improves performance', t => {
  ...
  // First pass - populate cache
  const start1 = performance.now()
  const levels1 = reputationValues.map(rep => ReputationManager.getReputationLevel(rep))
  const time1 = performance.now() - start1
  
  // Second pass - use cache
  const start2 = performance.now()
  const levels2 = reputationValues.map(rep => ReputationManager.getReputationLevel(rep))
  const time2 = performance.now() - start2
...
test('ReputationManager location reputation caching improves performance', t => {
  ...
  const locations = ['Market Square', 'Royal Castle', 'Enchanted Forest', 'Peasant Village']
  
  // First pass - populate cache
  const start1 = performance.now()
  const locationReps1 = locations.map(loc => ReputationManager.getLocationReputation(reputationState, loc))
  const time1 = performance.now() - start1
  
  // Second pass - use cache
  const start2 = performance.now()
  const locationReps2 = locations.map(loc => ReputationManager.getLocationReputation(reputationState, loc))
  const time2 = performance.now() - start2
...
test('ReputationManager cache invalidation works with state changes', t => {
  ...
  // First calculation
  const locationRep1 = ReputationManager.getLocationReputation(reputationState, 'Market Square')
  ...
  const locationRep2 = ReputationManager.getLocationReputation(reputationState, 'Market Square')
...
test('ReputationManager memoization accuracy', t => {
  ...
  const testCases = [
    ...
    { reputation: 0, expectedModifier: 1.0 },
  ...
  for (const testCase of testCases) {
    ...
    const modifier1 = ReputationManager.calculatePriceModifier(testCase.reputation)
    
    // Second call (cached)
    const modifier2 = ReputationManager.calculatePriceModifier(testCase.reputation)
...
test('ReputationManager complex reputation calculations with caching', t => {
  ...
  const npcRep1 = ReputationManager.getNPCReputation(reputationState, npcId, location)
  const npcRep2 = ReputationManager.getNPCReputation(reputationState, npcId, location)
