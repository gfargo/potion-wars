...
import { RivalAlchemistManager, type RivalAlchemist, type RivalEncounterContext } from '../RivalAlchemist.js'
...
test('RivalAlchemistManager getAllRivals', t => {
  ...
  const rival2 = { ...createMockRival(), id: 'test_rival_2', personality: { ...createMockRival().personality, name: 'Test Rival 2' } }
  ...
  t.true(allRivals.some(r => r.id === 'test_rival'), 'Should include first rival')
  t.true(allRivals.some(r => r.id === 'test_rival_2'), 'Should include second rival')
})
...
test('RivalAlchemistManager encounter probability calculation', t => {
  ...
  for (let i = 0; i < trials; i++) {
    const encounter = manager.rollForRivalEncounter("Alchemist's Quarter", gameState)
    if (encounter) {
      encounterCount++
    }
  }
...
test('RivalAlchemistManager no encounter in location without rivals', t => {
  ...
  t.is(encounter, null, 'Should not encounter rivals in locations where none are active')
})
...
test('RivalAlchemistManager price war resolution - player wins', t => {
  ...
  t.true(outcome.message.includes(rival.personality.name), 'Message should mention rival name')
})
...
test('RivalAlchemistManager sabotage resolution - player detects', t => {
  ...
  t.true(outcome.message.includes('caught'), 'Message should indicate detection')
})
...
test('RivalAlchemistManager sabotage resolution - player fails to detect', t => {
  ...
  t.true(outcome.message.includes('sabotaged'), 'Message should indicate sabotage success')
})
...
test('RivalAlchemistManager theft resolution - player defends', t => {
  ...
  t.true(outcome.message.includes('caught'), 'Message should indicate catching thief')
})
...
test('RivalAlchemistManager competition resolution', t => {
  ...
  t.true(typeof outcome.reputationChange.locationChange === 'number', 'Should have reputation change')
  t.true(typeof outcome.cashChange === 'number', 'Should have cash change')
  t.true(outcome.message.length > 0, 'Should have message')
  
  if (outcome.success) {
    t.true(outcome.reputationChange.locationChange! > 0, 'Winning should increase reputation')
    t.true(outcome.cashChange > 0, 'Winning should provide cash reward')
  } else {
    t.true(outcome.reputationChange.locationChange! < 0, 'Losing should decrease reputation')
...
test('RivalAlchemistManager market impact calculation', t => {
  ...
  const impacts = manager.calculateMarketImpact(rival, "Alchemist's Quarter", 10)
  
  t.true(impacts.length > 0, 'Should generate market impacts')
  t.true(impacts.every(impact => impact.type === 'rival'), 'All impacts should be rival type')
  t.true(impacts.every(impact => impact.location === "Alchemist's Quarter"), 'All impacts should be for correct location')
  t.true(impacts.every(impact => impact.startDay === 10), 'All impacts should start on correct day')
})
...
test('RivalAlchemistManager update rival after encounter', t => {
  ...
  t.is(updatedRival!.encounterHistory.length, 1, 'Should add encounter to history')
  t.deepEqual(updatedRival!.encounterHistory[0], encounterRecord, 'Should store encounter record')
  t.true(updatedRival!.reputation < originalReputation, 'Rival reputation should decrease after losing')
})
...
test('RivalAlchemistManager determine encounter type based on rival type', t => {
  ...
  t.true(encounterTypes.has('competition'), 'Should include base encounter types')
})
...
test('RivalAlchemistManager encounter history limit', t => {
  ...
  const updatedRival = manager.getRival('test_rival')
  t.is(updatedRival!.encounterHistory.length, 20, 'Should limit encounter history to 20 entries')
  t.is(updatedRival!.encounterHistory[0]!.day, 5, 'Should keep most recent encounters')
  t.is(updatedRival!.encounterHistory[19]!.day, 24, 'Should keep latest encounter')
})