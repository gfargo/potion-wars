...
import { type SupplyDemandFactor, type MarketState } from '../../types/economy.types.js'
import { type ReputationChange } from '../../types/reputation.types.js'

export type RivalAlchemistType = 'aggressive' | 'cunning' | 'merchant' | 'saboteur'
...
export type RivalEncounterType = 'price_war' | 'sabotage' | 'theft' | 'competition' | 'negotiation'
...
export class RivalAlchemistManager {
  ...
  private rivals: Map<string, RivalAlchemist> = new Map()

  static getInstance(): RivalAlchemistManager {
    if (!RivalAlchemistManager.instance) {
  ...
  // Encounter Logic
  rollForRivalEncounter(location: string, gameState: GameState): RivalAlchemist | null {
    ...
    if (Math.random() < encounterChance) {
      ...
      const totalWeight = weightedRivals.reduce((sum, item) => sum + item.weight, 0)
  ...
  private getActiveRivalsInLocation(location: string, gameState: GameState): RivalAlchemist[] {
  ...
  private calculateRivalWeight(rival: RivalAlchemist, gameState: GameState): number {
    let weight = 1
  ...
  resolveEncounter(context: RivalEncounterContext, encounterType: RivalEncounterType, playerChoice?: string): RivalEncounterOutcome {
  ...
  private resolvePriceWar(context: RivalEncounterContext, _playerChoice?: string): RivalEncounterOutcome {
  ...
  private resolveSabotage(context: RivalEncounterContext, _playerChoice?: string): RivalEncounterOutcome {
    const { rival, location } = context
    ...
    if (playerDetects) {
      ...
      const potionTypes = ['Healing Potion', 'Strength Potion', 'Agility Potion', 'Intelligence Potion']
      const lostPotion = potionTypes[Math.floor(Math.random() * potionTypes.length)]
      if (lostPotion) {
  ...
  private resolveTheft(context: RivalEncounterContext, _playerChoice?: string): RivalEncounterOutcome {
    const { rival, location } = context
  ...
  private resolveCompetition(context: RivalEncounterContext, _playerChoice?: string): RivalEncounterOutcome {
    ...
    const rivalScore = (rival.strength + rival.cunning + rival.wealth) / 3 + Math.random() * 3
  ...
  private resolveNegotiation(context: RivalEncounterContext, playerChoice?: string): RivalEncounterOutcome {
    const { rival, location } = context
    ...
    if (playerSucceeds) {
      ...
      return {
        ...
        reputationChange: { location, locationChange: 3 },
        cashChange: -cost,
        message: `You successfully negotiated with ${rival.personality.name}! ${playerChoice === 'bribe' ? 'The bribe was expensive but effective.' : 'Your diplomatic skills paid off.'}`
      }
    } else {
      return {
        success: false,
        ...
        message: `Negotiations with ${rival.personality.name} failed! ${playerChoice === 'threaten' ? 'Your threats backfired.' : 'They saw through your approach.'}`
      }
    }
  }

  // Market Impact
  calculateMarketImpact(rival: RivalAlchemist, location: string, day: number): SupplyDemandFactor[] {
  ...
  updateRivalAfterEncounter(rivalId: string, encounter: RivalEncounterRecord): void {
  ...
  determineEncounterType(rival: RivalAlchemist, _context: RivalEncounterContext): RivalEncounterType {
