...
test('processChoice applies effects to game state', t => {
  ...
  manager.startDialogue(mockNPC, mockGameState)
  manager.processChoice(mockNPC, { text: 'I want to trade', nextNode: 'trade' }, mockGameState)
...
test('getCurrentNode returns correct node', t => {
  ...
  // No active dialogue
  t.is(manager.getCurrentNode(mockNPC), null)
...
test('validateChoice validates choices correctly', t => {
  ...
  // No active dialogue
  let validation = manager.validateChoice(mockNPC, { text: 'Test', nextNode: undefined }, mockGameState)
  ...
  // Valid choice
  validation = manager.validateChoice(mockNPC, { text: 'I want to trade', nextNode: 'trade' }, mockGameState)
  t.true(validation.isValid)
  
  // Invalid choice
  validation = manager.validateChoice(mockNPC, { text: 'Invalid', nextNode: 'nonexistent' }, mockGameState)
  t.false(validation.isValid)
...
test('validateChoice checks choice conditions', t => {
  ...
  const validation = manager.validateChoice(mockNPC, { 
    text: 'Tell me about the market', 
    nextNode: 'info',
    conditions: [{ type: 'reputation', operator: 'gte', value: 20 }]
  }, lowRepState)
...
test('endDialogue ends active dialogue', t => {
  ...
  manager.endDialogue()
  t.false(manager.isDialogueActive())
  t.is(manager.getDialogueState(), null)
})
...
test('getDialogueStats returns correct statistics', t => {
  ...
  // No active dialogue
  t.is(manager.getDialogueStats(), null)
