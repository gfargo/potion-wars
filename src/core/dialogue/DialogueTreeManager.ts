...
export class DialogueTreeManager {
  ...
  static getInstance(): DialogueTreeManager {
    if (!DialogueTreeManager.instance) {
  ...
  startDialogue(npc: NPC, gameState: GameState): DialogueNode {
    ...
    if (validationErrors.length > 0) {
      throw new Error(`Invalid dialogue tree for NPC ${npc.id}: ${validationErrors.join(', ')}`)
    }
   ...
   */
  processChoice(npc: NPC, choice: DialogueChoice, gameState: GameState): {
    ...
    if (this.activeDialogue.npcId !== npc.id) {
      throw new Error(`Active dialogue is with different NPC (${this.activeDialogue.npcId})`)
    }

    // Validate the choice belongs to the current node
    const currentNode = npc.dialogue.nodes[this.activeDialogue.currentNodeId]
    if (!currentNode) {
      throw new Error(`Current dialogue node '${this.activeDialogue.currentNodeId}' not found`)
    }

    const isValidChoice = currentNode.choices.some(nodeChoice => 
      nodeChoice.text === choice.text && nodeChoice.nextNode === choice.nextNode
    )
    ...
    const newGameState = DialogueEngine.handleChoice(choice, gameState, npc.location)
  ...
  isDialogueActive(): boolean {
    return this.activeDialogue !== null && this.activeDialogue.isActive
  }

  /**
   * Get the current dialogue node
   */
  getCurrentNode(npc: NPC): DialogueNode | null {
    if (!this.activeDialogue || !this.activeDialogue.isActive) {
      return null
    }
    ...
    return npc.dialogue.nodes[this.activeDialogue.currentNodeId] || null
  }
   ...
   */
  validateChoice(npc: NPC, choice: DialogueChoice, gameState: GameState): {
    isValid: boolean
    reason?: string
  } {
    if (!this.activeDialogue || !this.activeDialogue.isActive) {
    ...
    const nodeChoice = currentNode.choices.find(nodeChoice => 
      nodeChoice.text === choice.text && nodeChoice.nextNode === choice.nextNode
    )
    ...
    if (nodeChoice.conditions) {
      const conditionsMet = DialogueEngine.evaluateConditions(nodeChoice.conditions, gameState, npc.location)
      if (!conditionsMet) {
  ...
  getAvailableChoices(npc: NPC, gameState: GameState): DialogueChoice[] {
    ...
    return currentNode.choices.filter(choice => {
      ...
      return DialogueEngine.evaluateConditions(choice.conditions, gameState, npc.location)
    })
  }
  ...
  getDialogueStats(): {
    ...
    const startTime = this.activeDialogue.conversationHistory[0]?.timestamp || Date.now()
