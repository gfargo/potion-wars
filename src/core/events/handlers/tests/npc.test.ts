...
test('startNPCDialogue initializes dialogue event', (t) => {
  ...
  t.is(
    ...
    'Test Merchant: "Hello there! What can I do for you?"'
  )
...
test('handleNPCDialogueChoice processes choice and continues dialogue', (t) => {
  ...
  const gameState = dialogueResult.newGameState
  ...
  t.is(
    event.steps[1]!.description,
    'Test Merchant: "I have some fine wares for sale."'
  )
})
...
test('handleNPCDialogueChoice applies effects', (t) => {
  ...
  gameState = handleNPCDialogueChoice(
    mockNPC,
...
test('validateNPCEvent validates reputation requirements', (t) => {
  const npcWithRepRequest = {
    ...mockNPC,
    reputation: { minimum: 50 },
  }

  t.false(validateNPCEvent(npcWithRepRequest, mockGameState)) // Only has 25 reputation
  ...
  t.true(validateNPCEvent(npcWithRepRequest, highRepState))
})
...
test('getAvailableNPCEvents returns valid events', (t) => {
  ...
  t.true(eventIds.has('npc_encounter_test_merchant'))
  t.true(eventIds.has('npc_trade_test_merchant'))
  t.true(eventIds.has('npc_simple_test_merchant'))
})
...
test('trading event choice fails with insufficient reputation', (t) => {
  ...
  t.truthy(result._result?.message)
  t.true(
    result._result!.message!.includes("don't meet the reputation requirements")
  )
})
