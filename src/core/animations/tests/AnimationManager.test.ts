...
test('loadAnimations initializes default animations', async t => {
  ...
  t.true(stats.travelCount > 0, `Expected travelCount > 0, got ${stats.travelCount}`)
  t.true(stats.encounterCount > 0, `Expected encounterCount > 0, got ${stats.encounterCount}`)
})
