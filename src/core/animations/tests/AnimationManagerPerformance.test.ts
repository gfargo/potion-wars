...
import type { NPCAnimation, TravelAnimation, EncounterAnimation, AnimationFrame } from '../../../types/animation.types.js'
...
test('AnimationManager NPC animation caching improves performance', async t => {
  ...
  t.true(time2 <= time1, `Cached call (${time2}ms) should be faster than or equal to initial call (${time1}ms)`)
...
test('AnimationManager optimization caching improves performance', async t => {
  ...
  t.true(time2 < time1, `Cached optimization (${time2}ms) should be faster than initial optimization (${time1}ms)`)
})
...
test('AnimationManager memory management with large animations', async t => {
  ...
  t.true(time < 200, `Loading 150 animation sets took ${time}ms, should be under 200ms`)
  
  // Memory management should prevent excessive memory usage
  const stats = manager.getMemoryStats()
  t.true(stats.totalCacheSize <= 100, `Cache size ${stats.totalCacheSize} should be limited`)
})
...
test('AnimationManager cache cleanup and memory limits', async t => {
  ...
  t.true(stats.totalCacheSize <= 100, `Total cache size ${stats.totalCacheSize} should be limited to prevent memory leaks`)
})
...
test('AnimationManager performance with default animations', async t => {
  ...
  t.true(time < 100, `400 default animation calls took ${time}ms, should be under 100ms`)
})
...
test('AnimationManager validation performance', async t => {
  ...
  t.true(time1 < 50, `Validation of 100 frames took ${time1}ms, should be under 50ms`)
  t.true(time2 < 10, `Validation of invalid frames took ${time2}ms, should be under 10ms`)
...
test('AnimationManager random travel animation performance', async t => {
  ...
  // Should complete quickly
  t.true(time < 50, `100 random travel animation selections took ${time}ms, should be under 50ms`)
  
  // Should return valid animations
  t.is(animations.length, 100)
  t.true(animations.every(anim => anim.name && anim.frames.length > 0))
})
...
test('AnimationManager concurrent access performance', async t => {
  ...
  t.true(time < 100, `50 concurrent animation accesses took ${time}ms, should be under 100ms`)
})
...
test('AnimationManager cache expiry and cleanup', async t => {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    manager.optimizeAnimationData(createLargeAnimationFrames(5), { removeEmptyFrames: true })
  }
