...
import { generateDynamicPrices, processPurchase, processSale, updateDailyMarkets, getLocationMarketTrends, initializeGameMarkets } from '../economy.js'
  ...
  marketData: {
    "Merchant's District": {
      ...
      'Strength Tonic': {
        ...
        trend: 'falling',
        history: [
          { day: 1, price: 160, volume: 5, playerTransaction: false }
        ],
        volatility: 0.15,
        lastUpdated: 1
      }
...
test('generateDynamicPrices applies reputation discounts', t => {
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    neutralTotal += neutralPrices['Wisdom Draught']! + neutralPrices['Strength Tonic']!
    discountedTotal += discountedPrices['Wisdom Draught']! + discountedPrices['Strength Tonic']!
  }
  ...
  t.true(discountedAverage < neutralAverage, `Discounted average (${discountedAverage}) should be less than neutral average (${neutralAverage})`)
})
...
test('processPurchase updates inventory, cash, and market data', t => {
  ...
  const marketData = updatedState.marketData["Merchant's District"]?.['Wisdom Draught']
  t.truthy(marketData)
