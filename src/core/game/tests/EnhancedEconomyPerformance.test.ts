...
import { type MarketData, type PriceHistoryEntry } from '../../../types/economy.types.js'
...
const createLongPriceHistory = (days: number, basePrice = 100): PriceHistoryEntry[] => {
...
test('EnhancedEconomyManager price calculation caching improves performance', t => {
  ...
  const price1 = EnhancedEconomyManager.calculateDynamicPrice(marketData, reputationModifier)
  const time1 = performance.now() - start1
  
  // Second calculation - use cache
  const start2 = performance.now()
  const price2 = EnhancedEconomyManager.calculateDynamicPrice(marketData, reputationModifier)
...
test('EnhancedEconomyManager market trend caching improves performance', t => {
  ...
  // First calculation - populate cache
  const start1 = performance.now()
  const trend1 = EnhancedEconomyManager.calculateMarketTrend(history, currentPrice)
  const time1 = performance.now() - start1
  
  // Second calculation - use cache
  const start2 = performance.now()
  const trend2 = EnhancedEconomyManager.calculateMarketTrend(history, currentPrice)
  const time2 = performance.now() - start2
...
test('EnhancedEconomyManager cache invalidation with different parameters', t => {
  ...
  const price1_cached = EnhancedEconomyManager.calculateDynamicPrice(marketData, 0.8)
  const price2_cached = EnhancedEconomyManager.calculateDynamicPrice(marketData, 1.2)
...
test('EnhancedEconomyManager performance with complex market data', t => {
  ...
  t.true(time < 100, `100 price calculations took ${time}ms, should be under 100ms`)
})
...
test('EnhancedEconomyManager trend calculation performance with large history', t => {
  ...
  const start = performance.now()
  const trend = EnhancedEconomyManager.calculateMarketTrend(longHistory, currentPrice)
  const time = performance.now() - start
  
  // Should complete in reasonable time even with large history
  t.true(time < 50, `Trend calculation with 1000 history entries took ${time}ms, should be under 50ms`)
...
test('EnhancedEconomyManager cache cleanup prevents memory leaks', t => {
  ...
  for (let i = 0; i < 1000; i++) {
    ...
    EnhancedEconomyManager.calculateDynamicPrice(marketData, 1.0)
  }
...
test('EnhancedEconomyManager deterministic pricing with caching', t => {
  ...
  const prices = []
  for (let i = 0; i < 10; i++) {
    prices.push(EnhancedEconomyManager.calculateDynamicPrice(marketData, 1.0))
  }
...
test('EnhancedEconomyManager cache TTL expiration', async t => {
  ...
  // First calculation
  const price1 = EnhancedEconomyManager.calculateDynamicPrice(marketData, 1.0)
  ...
  const price2 = EnhancedEconomyManager.calculateDynamicPrice(marketData, 1.0)
...
test('EnhancedEconomyManager market trends with various patterns', t => {
  ...
  for (const pattern of patterns) {
    ...
    const currentPrice = pattern[pattern.length - 1]!
...
test('EnhancedEconomyManager initialization performance', t => {
  ...
  t.true(time < 50, `Market initialization took ${time}ms, should be under 50ms`)
  ...
  for (const location of locations) {
    ...
    t.true(marketData[location] !== undefined && Object.keys(marketData[location]!).length > 0)
  }
})