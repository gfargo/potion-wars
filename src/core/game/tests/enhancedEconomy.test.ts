...
test('initializeMarketData creates market data for all locations', (t) => {
  ...
  for (const location of expectedLocations) {
    ...
    t.true(
      typeof marketData[location] === 'object',
      `${location} should have market data object`
    )
  }
})
...
test('recordTransaction updates market data correctly', (t) => {
  ...
  const updatedMarket = EnhancedEconomyManager.recordTransaction(
    initialMarket,
    5,
    2,
    true
...
test('calculateDynamicPrice respects supply and demand', (t) => {
  ...
  const highDemandPrice = EnhancedEconomyManager.calculateDynamicPrice(
    highDemandMarket,
    1
  )
  const lowDemandPrice = EnhancedEconomyManager.calculateDynamicPrice(
    lowDemandMarket,
    1
  )
...
test('calculateDynamicPrice applies reputation modifier', (t) => {
  ...
  const normalPrice = EnhancedEconomyManager.calculateDynamicPrice(
    marketData,
    1
  )
  const discountPrice = EnhancedEconomyManager.calculateDynamicPrice(
    marketData,
    0.8
  )
  const premiumPrice = EnhancedEconomyManager.calculateDynamicPrice(
    marketData,
    1.2
  )
...
test('calculateMarketTrend identifies trends correctly', (t) => {
  ...
  const risingTrend = EnhancedEconomyManager.calculateMarketTrend(
    risingHistory,
    140
  )
  ...
  const fallingTrend = EnhancedEconomyManager.calculateMarketTrend(
    fallingHistory,
    100
  )
  ...
  const stableTrend = EnhancedEconomyManager.calculateMarketTrend(
    stableHistory,
    99
  )
  ...
  const volatileTrend = EnhancedEconomyManager.calculateMarketTrend(
    volatileHistory,
    90
  )
...
test('applySupplyDemandFactors modifies market correctly', (t) => {
  ...
  const updatedState = EnhancedEconomyManager.applySupplyDemandFactors(
    mockState as GameState,
    factors
  )

  const updatedMarket =
    updatedState.marketData['Market Square']?.['Healing Potion']
