...
import { checkNPCEncounter, getLocationNPCs, getLocationNPCsByType } from '../travel.js'
...
test.serial('getLocationNPCsByType returns empty array for non-existent type', t => {
...
test.serial('travel integration works with different locations', t => {
  ...
  const merchantDistrictState = {
    ...mockGameState,
    location: { name: "Merchant's District", description: 'Busy marketplace', dangerLevel: 3 }
  }
  ...
  const castleState = {
    ...mockGameState,
    location: { name: 'Royal Castle', description: 'Seat of power', dangerLevel: 5 },
    reputation: {
      global: 30,
  ...
  const forestState = {
    ...mockGameState,
    location: { name: 'Enchanted Forest', description: 'Magical woodland', dangerLevel: 4 }
  }
...
test.serial('NPC type filtering works correctly across locations', t => {
  ...
  const merchantDistrictMerchants = getLocationNPCsByType(mockGameState, 'merchant')
