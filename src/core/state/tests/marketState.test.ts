...
test('recordTransaction action updates market and trade history', (t) => {
  ...
  const marketData =
    newState.marketData["Merchant's District"]?.['Wisdom Draught']
  t.truthy(marketData)
  t.true(
    marketData!.demand >
...
test('applySupplyDemandFactors action modifies market', (t) => {
  ...
  const originalSupply =
    state.marketData["Merchant's District"]!['Wisdom Draught']!.supply
  ...
  t.true(
    Math.abs(marketData!.demand - (originalDemand + expectedDemandIncrease)) <
      0.01
  )
  t.true(
    Math.abs(marketData!.supply - (originalSupply + expectedSupplyDecrease)) <
      0.01
  )
})
...
test('selectPotionCurrentPrice returns current price', (t) => {
  const state = createInitialState()
  const price = selectPotionCurrentPrice(
    state,
    'Wisdom Draught',
    "Merchant's District"
  )
...
test('selectBestBuyingOpportunities identifies buying opportunities', (t) => {
  ...
  const opportunities = selectBestBuyingOpportunities(
    state,
    "Merchant's District"
  )

  t.true(Array.isArray(opportunities))
  const fallingPotion = opportunities.find(
    (opp) => opp.potionType === 'Strength Tonic'
  )
