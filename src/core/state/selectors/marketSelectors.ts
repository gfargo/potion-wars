...
export const selectLocationMarketData = (
  state: GameState,
  location?: string
...
export const selectCurrentLocationMarketData = (
  state: GameState
): MarketState | undefined => state.marketData[state.location.name]
...
export const selectCurrentPotionMarketData = (
  state: GameState,
  potionType: string
): MarketData | undefined => state.marketData[state.location.name]?.[potionType]
...
export const selectLocationMarketTrends = (
  state: GameState,
  location?: string
) => {
  const targetLocation = location || state.location.name
...
export const selectPotionRecentPriceChange = (
  ...
  const currentPrice = history.at(-1)?.price || 0
  const previousPrice = history.at(-2)?.price || 0
...
export const selectMarketConditions = (state: GameState, location?: string) => {
  ...
  const trendCounts = trends.reduce<Record<MarketTrend, number>>(
    (accumulator, trend) => {
      accumulator[trend.trend] = (accumulator[trend.trend] || 0) + 1
      return accumulator
    },
    { rising: 0, falling: 0, stable: 0, volatile: 0 } as Record<MarketTrend, number>
  )
  ...
  return {
    ...
    overallTrend: Object.entries(trendCounts).reduce((a, b) =>
      trendCounts[a[0] as MarketTrend] > trendCounts[b[0] as MarketTrend]
        ? a
        : b
    )[0] as MarketTrend,
  }
...
// Market Intelligence
export const selectBestBuyingOpportunities = (
  state: GameState,
  location?: string
) => {
  const trends = selectLocationMarketTrends(state, location)
  ...
  return trends
    .filter(
      (trend) =>
        trend.trend === 'falling' ||
        (trend.trend === 'stable' && trend.priceChange < -2)
    )
    .sort((a, b) => a.priceChange - b.priceChange) // Most negative change first
    .slice(0, 3)
}

export const selectBestSellingOpportunities = (
  state: GameState,
  location?: string
) => {
  const trends = selectLocationMarketTrends(state, location)
...
export const selectMarketOpportunities = (
  state: GameState,
  location?: string
) => ({
...
// Market Status
export const selectIsMarketDataStale = (
  state: GameState,
  location?: string
): boolean => {
  const targetLocation = location || state.location.name
  ...
  return Object.values(locationMarket).some(
    (marketData) => state.day - marketData.lastUpdated > 1
  )
}

export const selectMarketDataAge = (
  state: GameState,
  location?: string
): number => {
  const targetLocation = location || state.location.name
  ...
  return Math.max(
    ...Object.values(locationMarket).map(
      (marketData) => state.day - marketData.lastUpdated
    )
  )
}

// Comparative Analysis
export const selectLocationPriceComparison = (
  state: GameState,
  potionType: string
) => {
  const comparison = []
...
export const selectBestLocationToBuy = (
  state: GameState,
  potionType: string
) => {
...
export const selectBestLocationToSell = (
  state: GameState,
  potionType: string
) => {
  const comparison = selectLocationPriceComparison(state, potionType)
  return comparison.at(-1) // Highest price
}
