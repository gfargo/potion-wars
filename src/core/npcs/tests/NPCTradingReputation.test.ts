...
const createGameStateWithReputation = (
  ...
  agility: 10,
  intelligence: 10,
  location: { name: 'Reputation Market', description: 'A test market', dangerLevel: 1 },
...
test('exclusive trades require minimum reputation', t => {
  ...
  t.deepEqual(highRepItems, ['Common Potion', 'Exclusive Potion', 'Ultra Rare Potion'])
})
...
test('successful trades increase reputation at multiple levels', t => {
  ...
  t.true((result.newGameState.reputation.locations['Reputation Market'] || 0) > 
         (gameState.reputation.locations['Reputation Market'] || 0))
  t.true((result.newGameState.reputation.npcRelationships['reputation_merchant'] || 0) > 
         (gameState.reputation.npcRelationships['reputation_merchant'] || 0))
})
...
test('reputation levels affect trade availability and pricing consistently', t => {
  ...
  for (const { rep, level } of reputationLevels) {
    ...
    if (previousPrice > 0 && rep > -30) { // Skip the most negative comparisons
      t.true(currentPrice <= previousPrice, 
