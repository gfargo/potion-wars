...
test.serial('getNPCsForLocation returns NPCs in the specified location', t => {
  const manager = NPCManager.getInstance()
  ...
  const marketNPCs = manager.getNPCsForLocation("Merchant's District", mockGameState)
...
test.serial('isNPCAvailable checks NPC-specific reputation requirements', t => {
  const manager = NPCManager.getInstance()
...
test.serial('isNPCAvailable uses global reputation when no location specified', t => {
  const manager = NPCManager.getInstance()
  ...
  t.false(manager.isNPCAvailable(npcWithGlobalRepRequirement, lowRepGameState))
})
...
test.serial('rollForEncounter respects probability weights', t => {
  ...
  const npc1 = { ...mockNPC, id: 'npc1', availability: { ...mockNPC.availability, probability: 0.1 } }
  const npc2 = { ...mockNPC, id: 'npc2', availability: { ...mockNPC.availability, probability: 0.9 } }
