...
import { NPCMarketIntelligence, type MarketTrend, type MarketAdvice } from '../NPCMarketIntelligence.js'
...
test('generateMarketTrends analyzes price trends correctly', t => {
  ...
  const healingTrend = trends.find(trend => 
...
test('generateMarketTrends filters by location', t => {
  const trends = NPCMarketIntelligence.generateMarketTrends(mockGameState, 'Market Square')
...
test('generateMarketAdvice provides buying advice for low prices', t => {
  const advice = NPCMarketIntelligence.generateMarketAdvice(mockGameState, 'Royal Palace')
...
test('generateMarketAdvice provides selling advice for high prices', t => {
  const advice = NPCMarketIntelligence.generateMarketAdvice(mockGameState, 'Market Square')
...
test('generateMarketAdvice considers player inventory', t => {
  ...
  const advice = NPCMarketIntelligence.generateMarketAdvice(noInventoryState, 'Market Square')
  const healingAdvice = advice.find(adv => adv.potion === 'Healing Potion')
...
test('generateMarketAdvice adjusts confidence based on reputation', t => {
  ...
  const lowRepAdvice = NPCMarketIntelligence.generateMarketAdvice(mockGameState, 'Market Square')
  const highRepAdvice = NPCMarketIntelligence.generateMarketAdvice(highRepState, 'Market Square')
  
  const lowRepHealing = lowRepAdvice.find(adv => adv.potion === 'Healing Potion')
  const highRepHealing = highRepAdvice.find(adv => adv.potion === 'Healing Potion')
...
test('getLocationIntelligence generates trend information', t => {
  const intelligence = NPCMarketIntelligence.getLocationIntelligence(mockGameState, 'Market Square')
  
  const trendInfo = intelligence.find(info => info.id.includes('market_trends'))
  t.truthy(trendInfo)
...
test('getLocationIntelligence generates advice information', t => {
  ...
  const adviceInfo = intelligence.find(info => info.id.includes('market_advice'))
  t.truthy(adviceInfo)
...
test('getLocationIntelligence filters by confidence', t => {
  ...
  const intelligence = NPCMarketIntelligence.getLocationIntelligence(lowConfidenceState, 'Market Square')
...
test('analyzePriceTrend handles insufficient data', t => {
  ...
  const trends = NPCMarketIntelligence.generateMarketTrends(gameStateWithShortHistory)
...
test('analyzeMarketOpportunity considers player cash', t => {
  ...
  const advice = NPCMarketIntelligence.generateMarketAdvice(poorPlayerState, 'Royal Palace')
  const healingAdvice = advice.find(adv => adv.potion === 'Healing Potion')
  
  if (healingAdvice && healingAdvice.expectedProfit) {
