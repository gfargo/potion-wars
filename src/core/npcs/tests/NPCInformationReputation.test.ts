...
import { NPCInformationReputation, type InformationTier } from '../NPCInformationReputation.js'
...
test('filterInformationByReputation filters based on effective reputation', t => {
  ...
  const filteredInfo = NPCInformationReputation.filterInformationByReputation(mockNPC, mockGameState)
...
test('filterInformationByReputation uses highest relevant reputation', t => {
  ...
  const filteredInfo = NPCInformationReputation.filterInformationByReputation(highRepNPC, mockGameState)
  
  // Debug: check what information is available
  const exclusiveInfo = filteredInfo.find(info => info.id === 'exclusive_info')
...
test('enhanceInformationContent adds details based on reputation tier', t => {
  ...
  const basicEnhanced = NPCInformationReputation.enhanceInformationContent(basicInfo, 10, 'test_npc')
  const detailedEnhanced = NPCInformationReputation.enhanceInformationContent(basicInfo, 30, 'test_npc')
  const exclusiveEnhanced = NPCInformationReputation.enhanceInformationContent(basicInfo, 60, 'test_npc')
  const secretEnhanced = NPCInformationReputation.enhanceInformationContent(basicInfo, 90, 'test_npc')
...
test('generateReputationBasedMarketIntelligence filters by reputation', t => {
  const intelligence = NPCInformationReputation.generateReputationBasedMarketIntelligence(
    mockGameState,
    'Market Square',
    'test_npc'
  )
...
test('calculateInformationReputationReward calculates correct rewards', t => {
  ...
  const basicReward = NPCInformationReputation.calculateInformationReputationReward(
    basicInfo, 'test_npc', mockGameState, false
  )
  const exclusiveReward = NPCInformationReputation.calculateInformationReputationReward(
    exclusiveInfo, 'test_npc', mockGameState, false
  )
  const firstTimeReward = NPCInformationReputation.calculateInformationReputationReward(
    basicInfo, 'test_npc', mockGameState, true
  )
...
test('applyInformationReward updates reputation correctly', t => {
  ...
  const initialNpcRep = initialState.reputation.npcRelationships['test_npc'] || 0
  t.true(npcRep > initialNpcRep)
})

test('hasAccessedInformation provides reasonable heuristic', t => {
  const earlyGameState = { ...mockGameState, day: 2, reputation: { ...mockGameState.reputation, global: 5 } }
  const lateGameState = { ...mockGameState, day: 10, reputation: { ...mockGameState.reputation, global: 25 } }
  
  // Early game with low reputation should return false
  t.false(NPCInformationReputation.hasAccessedInformation(earlyGameState, 'test_info'))
  
  // Later game with higher reputation should return true
  t.true(NPCInformationReputation.hasAccessedInformation(lateGameState, 'test_info'))
})
...
test('getExclusiveOpportunities returns opportunities based on high reputation', t => {
  ...
  const opportunities = NPCInformationReputation.getExclusiveOpportunities(highRepState, 'Royal Palace')
...
test('getExclusiveOpportunities returns empty array for low reputation', t => {
  ...
  const opportunities = NPCInformationReputation.getExclusiveOpportunities(lowRepState, 'Dark Alley')
...
test('enhanced content includes tier-appropriate details', t => {
  ...
  const detailedEnhanced = NPCInformationReputation.enhanceInformationContent(baseInfo, 30, 'test_npc')
  const exclusiveEnhanced = NPCInformationReputation.enhanceInformationContent(baseInfo, 60, 'test_npc')
  const secretEnhanced = NPCInformationReputation.enhanceInformationContent(baseInfo, 90, 'test_npc')
...
test('market intelligence enhancement adds specific details', t => {
  const baseIntelligence = NPCInformationReputation.generateReputationBasedMarketIntelligence(
    mockGameState,
    'Market Square',
    'test_npc'
  )
  ...
  const enhancedIntelligence = NPCInformationReputation.generateReputationBasedMarketIntelligence(
    highRepState,
    'Market Square',
    'test_npc'
  )
