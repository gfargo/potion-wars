...
import { NPCInformationManager, NPCInformationError } from '../NPCInformationManager.js'
...
const mockInformation: NPCInformation[] = [
  ...
  },
  {
    ...
    content: 'A wealthy merchant is looking for rare potions. He pays triple the normal price.',
    category: 'market',
...
test('getAvailableInformation returns all accessible information', t => {
  const available = NPCInformationManager.getAvailableInformation(mockNPC, mockGameState)
...
test('getAvailableInformation returns empty array for NPC without information', t => {
  ...
  const available = NPCInformationManager.getAvailableInformation(npcWithoutInfo, mockGameState)
  
  t.is(available.length, 0)
})

test('getInformationByCategory filters by category correctly', t => {
  const marketInfo = NPCInformationManager.getInformationByCategory(mockNPC, 'market', mockGameState)
  const eventInfo = NPCInformationManager.getInformationByCategory(mockNPC, 'event', mockGameState)
...
test('isInformationAvailable checks reputation requirements', t => {
  ...
  t.true(NPCInformationManager.isInformationAvailable(basicInfo, mockGameState, 'Market Square'))
  t.false(NPCInformationManager.isInformationAvailable(exclusiveInfo, mockGameState, 'Market Square'))
})
...
test('isInformationAvailable evaluates conditions correctly', t => {
  ...
  t.true(NPCInformationManager.isInformationAvailable(eventInfo, mockGameState, 'Market Square'))
  t.true(NPCInformationManager.isInformationAvailable(inventoryInfo, mockGameState, 'Market Square'))
  
  // Test with early day
  const earlyGameState = { ...mockGameState, day: 3 }
  t.false(NPCInformationManager.isInformationAvailable(eventInfo, earlyGameState, 'Market Square'))
  
  // Test without required item
  const noItemGameState = { ...mockGameState, inventory: {} }
  t.false(NPCInformationManager.isInformationAvailable(inventoryInfo, noItemGameState, 'Market Square'))
})
...
test('evaluateInformationConditions handles all condition types', t => {
  ...
  t.true(NPCInformationManager.evaluateInformationConditions(conditions, mockGameState, 'Market Square'))
  ...
  t.false(NPCInformationManager.evaluateInformationConditions(failingConditions, mockGameState, 'Market Square'))
})
...
test('evaluateInformationCondition throws error for invalid inventory condition', t => {
  ...
  const error = t.throws(() => {
    NPCInformationManager.evaluateInformationConditions([invalidCondition], mockGameState, 'Market Square')
...
test('filterInformationByQuality returns content', t => {
  ...
  t.is(NPCInformationManager.filterInformationByQuality(info, 'basic'), info.content)
  t.is(NPCInformationManager.filterInformationByQuality(info, 'detailed'), info.content)
  t.is(NPCInformationManager.filterInformationByQuality(info, 'exclusive'), info.content)
})
...
test('validateInformation catches invalid data', t => {
  ...
  const contentErrors = NPCInformationManager.validateInformation(invalidContentInfo)
  t.true(contentErrors.some(error => error.includes('content')))
  
  // Test invalid category
  const invalidCategoryInfo = { ...validInfo, category: 'invalid' as any }
  const categoryErrors = NPCInformationManager.validateInformation(invalidCategoryInfo)
  ...
  const reputationErrors = NPCInformationManager.validateInformation(invalidReputationInfo)
  t.true(reputationErrors.some(error => error.includes('Reputation requirement')))
})
...
test('validateInformation validates conditions', t => {
  ...
  const errors = NPCInformationManager.validateInformation(infoWithInvalidConditions)
...
test('compareValues handles all operators correctly', t => {
  ...
  t.true(NPCInformationManager['compareValues']('Market Square', 'eq', 'Market Square'))
  t.false(NPCInformationManager['compareValues']('Market Square', 'eq', 'Dark Alley'))
})
