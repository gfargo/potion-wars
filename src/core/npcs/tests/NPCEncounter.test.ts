...
test.serial('checkForEncounter returns NPC or undefined', (t) => {
...
test.serial(
  'getAvailableNPCs returns empty array for location with no NPCs',
  (t) => {
...
test.serial('isNPCAvailable checks NPC availability', (t) => {
  ...
  const isAvailable = NPCEncounter.isNPCAvailable(
    'merchant_aldric',
    mockGameState
  )
...
test.serial('reset clears NPCs and resets initialization', (t) => {
  ...
  t.is(
    afterReinitCount,
    initialCount,
    'Should have same count after re-initialization'
  )
})
