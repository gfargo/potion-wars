...
test.serial(
  'validateNPCData catches missing root node in dialogue nodes',
  (t) => {
    const invalidData = {
      ...validNPCData,
      dialogue: {
    ...
    t.true(
      errors.some(
        (e) => e.field === 'dialogue.nodes' && e.message.includes('Root node')
      )
    )
  }
)
...
test.serial('getDefaultNPCs returns valid NPC array', (t) => {
  ...
  for (const npc of defaultNPCs) {
    ...
    t.is(
      errors.length,
      0,
      `Default NPC ${npc.id} has validation errors: ${errors
        .map((e) => e.message)
        .join(', ')}`
    )
  }
...
test.serial('default NPCs have proper dialogue structure', (t) => {
  ...
  for (const npc of defaultNPCs) {
    ...
    t.truthy(
      npc.dialogue.nodes[npc.dialogue.rootNode],
      `NPC ${npc.id} root node should exist in nodes`
    )
    ...
    for (const [nodeId, node] of Object.entries(npc.dialogue.nodes)) {
      ...
      t.true(
        ...
        `Node choices should be array for ${npc.id}.${nodeId}`
      )
    }
  }
})
...
test.serial('default merchant NPCs have trades', (t) => {
  ...
  for (const merchant of merchants) {
    t.true(
      Array.isArray(merchant.trades),
      `Merchant ${merchant.id} should have trades array`
    )
    if (merchant.trades && merchant.trades.length > 0) {
      for (const trade of merchant.trades) {
        ...
        t.is(
          typeof trade.price,
          'number',
          `Trade price should be number for ${merchant.id}`
        )
        t.is(
          typeof trade.quantity,
          'number',
          `Trade quantity should be number for ${merchant.id}`
        )
      }
    }
  }
...
test.serial('default informant NPCs have information', (t) => {
  ...
  for (const informant of informants) {
    t.true(
      Array.isArray(informant.information),
      `Informant ${informant.id} should have information array`
    )
    if (informant.information && informant.information.length > 0) {
      for (const info of informant.information) {
        ...
        t.truthy(
          info.content,
          `Information should have content for ${informant.id}`
        )
        t.truthy(
          info.category,
          `Information should have category for ${informant.id}`
        )
      }
    }
  }
