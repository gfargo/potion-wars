...
export class NPCInformationReputation {
  ...
  private static readonly REPUTATION_GATES: ReputationGate[] = [
    { tier: 'basic', minimumReputation: -20, description: 'Common knowledge available to most' },
    { tier: 'detailed', minimumReputation: 20, description: 'Trusted individuals get better details' },
    { tier: 'exclusive', minimumReputation: 50, description: 'Only well-respected people learn this' },
    { tier: 'secret', minimumReputation: 80, description: 'Reserved for the most trusted allies' }
  ]

  // Base rewards for receiving information
  private static readonly INFORMATION_REWARDS: Record<NPCInformation['category'], InformationReward> = {
    market: { baseReward: 2, categoryMultiplier: 1.5, tierMultiplier: 1.2, firstTimeBonus: 1 },
    event: { baseReward: 1, categoryMultiplier: 1.0, tierMultiplier: 1.1, firstTimeBonus: 2 },
    location: { baseReward: 1, categoryMultiplier: 1.0, tierMultiplier: 1.1, firstTimeBonus: 1 },
    general: { baseReward: 0.5, categoryMultiplier: 0.8, tierMultiplier: 1.0, firstTimeBonus: 0.5 }
  }
  ...
  static filterInformationByReputation(
    ...
    const effectiveReputation = Math.max(locationReputation, globalReputation * 0.5, npcReputation)
    ...
    return npc.information.filter(info => {
      ...
      if (info.conditions) {
        return NPCInformationManager.evaluateInformationConditions(info.conditions, gameState, npc.location)
      }
  ...
  static enhanceInformationContent(
    ...
    const enhancedContent = this.addReputationBasedDetails(information.content, tier, information.category)
  ...
  static generateReputationBasedMarketIntelligence(
    ...
    const baseIntelligence = NPCMarketIntelligence.getLocationIntelligence(gameState, location)
    ...
    return filteredIntelligence.map(info => this.enhanceMarketIntelligence(info, tier, gameState, location))
  }
  ...
  static calculateInformationReputationReward(
    ...
    let reward = rewards.baseReward * rewards.categoryMultiplier * tierMultiplier
    ...
    if (information.reputationRequirement && information.reputationRequirement > 40) {
      reward *= 1.5
    }
  ...
  static applyInformationReward(
    ...
    const reward = this.calculateInformationReputationReward(information, npcId, gameState, isFirstTime)
  ...
  private static getRequiredTier(information: NPCInformation): InformationTier {
    const repReq = information.reputationRequirement || 0
    
    if (repReq >= 80) return 'secret'
    if (repReq >= 50) return 'exclusive'
    if (repReq >= 20) return 'detailed'
    return 'basic'
  }
  ...
  private static getTierMultiplier(tier: InformationTier): number {
    switch (tier) {
      ...
      case 'detailed': return 1.2
      case 'exclusive': return 1.5
      case 'secret': return 2.0
      default: return 1.0
    }
  }
   ...
   */
  private static addDetailedContext(content: string, category: NPCInformation['category']): string {
    const details = {
   ...
   */
  private static addExclusiveInsights(content: string, category: NPCInformation['category']): string {
    const insights = {
      market: ' Additionally, I know of a merchant who will pay 150% for quality goods.',
      event: ' Between you and me, this will affect three other locations as well.',
      location: ' There\'s also a hidden cache of supplies there that few know about.',
   ...
   */
  private static addSecretInformation(content: string, category: NPCInformation['category']): string {
    const secrets = {
      market: ' Here\'s something only my closest allies know: there\'s going to be a royal decree affecting potion trade next week.',
      event: ' This is highly confidential, but the event is orchestrated by rival alchemists trying to manipulate the market.',
      location: ' I shouldn\'t tell you this, but there\'s a secret entrance that bypasses all the dangers.',
      general: ' Since you\'re like family to me, I\'ll tell you the whole truth behind this situation.'
    }
  ...
  private static enhanceMarketIntelligence(
    ...
    if (tier === 'detailed' || tier === 'exclusive' || tier === 'secret') {
      ...
      const trends = NPCMarketIntelligence.generateMarketTrends(gameState, location)
      if (trends.length > 0) {
    ...
    if (tier === 'exclusive' || tier === 'secret') {
      ...
      enhancedContent += ' The optimal time to act is during the morning hours when competition is lowest.'
    }
    
    if (tier === 'secret') {
      // Add insider knowledge
      enhancedContent += ' I also know of a private buyer who pays premium prices for bulk orders.'
    }
