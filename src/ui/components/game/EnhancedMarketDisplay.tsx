...
import { ContextualHelp, useContextualHelp } from '../common/ContextualHelp.js'

type EnhancedMarketDisplayProperties = {
...
export function EnhancedMarketDisplay({
  ...
  showHistory = false,
  compact = false,
}: EnhancedMarketDisplayProperties) {
  ...
  useEffect(() => {
    if (showTrends && !compact && !hasShownHint.current) {
    ...
    return (
      ...
      >
        ...
        {Object.entries(markets).map(([potionType, marketData]) => {
          const actualPrice = prices[potionType] ?? 0
          ...
          return (
            ...
            >
              ...
              <Box flexDirection="row" gap={1}>
                <Text color={getPriceColor(actualPrice, marketData.basePrice)}>
                  {actualPrice}g
                </Text>
        ...
        {reputationModifier !== 1 && (
          <Text dimColor>
            Rep bonus: {reputationModifier < 1 ? '-' : '+'}
            {Math.round(Math.abs(1 - reputationModifier) * 100)}%
          </Text>
        )}
      </Box>
    )
    ...
    <Box flexDirection="column" borderStyle="single" paddingX={1} minWidth={50}>
      ...
      <Box flexDirection="column" marginBottom={1}>
        <Text bold color="yellow">
          Current Prices ({currentLocation}):
        </Text>
        {Object.entries(markets).map(([potionType, marketData]) => {
          const actualPrice = prices[potionType] ?? 0
          const { trend } = marketData
          ...
          return (
            ...
            >
              <Box flexDirection="row" gap={1} width="60%">
                <Text>{potionType}:</Text>
                <Text dimColor color={getTrendColor(trend)}>
                  {getTrendIndicator(trend)}
                </Text>
              </Box>
              <Box flexDirection="row" gap={1}>
        ...
        <Box flexDirection="column" marginBottom={1}>
          ...
          {trends.map(({ potionType, trend, priceChange, confidence }) => (
            ...
            >
              <Text>{potionType}:</Text>
              <Box flexDirection="row" gap={1}>
                <Text color={getTrendColor(trend)}>{trend.toUpperCase()}</Text>
                {priceChange !== 0 && (
                ...
                <Text dimColor>({Math.round(confidence * 100)}%)</Text>
              </Box>
            </Box>
          ))}
        </Box>
      ...
      {reputationModifier !== 1 && (
        <Box marginTop={1} borderStyle="single" paddingX={1}>
          <Text color={reputationModifier < 1 ? 'green' : 'red'}>
            Reputation Effect: {reputationModifier < 1 ? 'Discount' : 'Markup'}{' '}
            of {Math.round(Math.abs(1 - reputationModifier) * 100)}%
          </Text>
        </Box>
      )}
...
function getMarketIntelligence(
  ...
  const risingTrends = trends.filter(
    (t) => t.trend === 'rising' && t.confidence > 0.6
  )
  const fallingTrends = trends.filter(
    (t) => t.trend === 'falling' && t.confidence > 0.6
  )
  ...
  if (risingTrends.length > 0) {
    ...
    if (best) {
      intelligence.push(
        `${
          best.potionType
        } prices are rising strongly (+${best.priceChange.toFixed(1)}%)`
      )
    }
  }
  ...
  if (fallingTrends.length > 0) {
    ...
    if (worst) {
      intelligence.push(
        `${worst.potionType} prices are falling (${worst.priceChange.toFixed(
          1
        )}%)`
      )
    }
  ...
  const avgTrend =
    trends.reduce((sum, t) => sum + t.priceChange, 0) / trends.length
  if (avgTrend > 5) {
...
export default React.memo(EnhancedMarketDisplay)
