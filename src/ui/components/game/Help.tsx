...
export default function Help() {
  ...
  const sections: HelpSection[] = ['basic', 'npcs', 'reputation', 'market', 'animations', 'advanced']
  ...
  useInput((input, key) => {
    if (input === 'h' || key.escape) {
      ...
      const sectionIndex = parseInt(input) - 1
  ...
  return (
    <Box flexDirection="column" borderStyle="classic" paddingX={1}>
      <Box justifyContent="center" marginBottom={1}>
        <Text bold color="cyan">{currentHelpSection.title}</Text>
      </Box>
      ...
      <Box flexDirection="column" borderStyle="single" paddingX={1}>
        <Box justifyContent="space-between" marginBottom={1}>
          ...
          <Text>{currentIndex + 1} of {sections.length}</Text>
        </Box>
        ...
        <Box marginTop={1} justifyContent="center">
          <Text>
            Use ← → arrows or numbers 1-6 to navigate • Press "h" or "esc" to close
          </Text>
        </Box>
      </Box>
    </Box>
