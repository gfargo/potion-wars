...
export function NPCPortrait({
  ...
  if (!isLoaded) {
    return <AsciiAnimation autoStart frames={getLoadingAnimation()} />
  }
...
function getAnimationSpeed(
  animationType: 'idle' | 'talking' | 'trading'
): number {
...
function shouldLoop(animationType: 'idle' | 'talking' | 'trading'): boolean {
  switch (animationType) {
    case 'idle': {
      return true
    } // Idle animations should loop continuously

    case 'talking': {
      return true
    } // Talking animations should loop during conversation

    case 'trading': {
      return false
    } // Trading animations might be one-shot
