...
test.beforeEach(() => {
  ...
  // Deeply reset state that may leak between tests via Immer draft
  useStore.setState((state) => {
    // Replace inventory entirely with a new object
    state.game.inventory = Object.create(null) as Record<string, number>
...
test('state updates are immediately visible after action call', (t) => {
  ...
  // Use setState to update (Immer-frozen state can't be mutated directly)
  useStore.setState((state) => {
    state.game.cash = initialCash + 1000
  })
...
test('brewPotion: state is immediately updated', (t) => {
  ...
  // Call action
  useStore.getState().brewPotion('Wisdom Draught', 5)
...
test('multiple sequential actions: all updates visible immediately', (t) => {
  // Use fresh setState to ensure clean inventory
  useStore.setState((state) => {
    state.game.inventory = {}
    state.game.prices['Wisdom Draught'] = 50
    state.game.prices['Strength Tonic'] = 100
    state.game.cash = 2000
  })

  // Verify clean starting state
  t.is(useStore.getState().game.inventory['Wisdom Draught'] ?? 0, 0, 'Inventory should start empty')

  // First action
  useStore.getState().brewPotion('Wisdom Draught', 3)

  // Immediately verify first action completed
  t.is(useStore.getState().game.inventory['Wisdom Draught'], 3)

  // Second action
  useStore.getState().brewPotion('Strength Tonic', 2)
...
test('complex sequence: travel -> event -> choice all synchronous', (t) => {
  ...
  // Clear any events generated by travel, then add our test event
  useStore.setState((state) => {
    state.events.queue = []
    state.events.current = undefined
    state.events.phase = 'choice'
