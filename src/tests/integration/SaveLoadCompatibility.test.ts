...
test('Legacy save file migration to modern format', (t) => {
  ...
  t.is(
    migratedState.reputation.global,
    0,
    'Global reputation should start at 0'
  )
  ...
  for (const location of locations) {
    t.truthy(
      migratedState.marketData[location],
      `Market data should exist for ${location}`
    )
  }
})
...
test('Save file validation with new features', (t) => {
  ...
  t.true(
    validateGameState(validState),
    'Valid modern state should pass validation'
  )

  // Invalid states should fail validation
  for (const invalidState of invalidStates) {
    t.false(
      validateGameState(invalidState as any),
...
test('NPC data persistence and restoration', (t) => {
  ...
  t.is(
    deserialized.reputation.npcRelationships[npc.id],
    25,
    'NPC relationship should be preserved'
  )
...
test('Market data persistence with price history', (t) => {
  ...
  t.is(
    deserializedMarketData.demand,
    updatedMarketData.demand,
    'Demand should be preserved'
  )
  t.is(
    deserializedMarketData.supply,
    updatedMarketData.supply,
  ...
  const dynamicPrice = EnhancedEconomyManager.calculateDynamicPrice(
    deserializedMarketData
  )
  t.true(
    dynamicPrice > 0,
    'Dynamic price calculation should work with loaded market data'
  )
})
...
test('Save file size optimization', (t) => {
  ...
  t.true(
    ...
    `Optimized save (${optimizedSize}) should be smaller than original (${originalSize})`
  )
  ...
  for (const location of Object.keys(optimizedState.marketData)) {
    for (const potion of Object.keys(optimizedState.marketData[location]!)) {
      ...
      const marketData = locationData[potion]!
      t.true(
        marketData.history.length <= 30,
        'Market history should be limited'
      )
    }
  }
})
...
test('Concurrent save/load operations', async (t) => {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    savePromises.push(saveGame(modifiedState, i + 1))
  }
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    t.is(
      loaded!.day,
      gameState.day + i - 1,
      `Save slot ${i} should have correct day`
    )
  }
})
