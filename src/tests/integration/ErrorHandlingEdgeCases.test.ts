...
test('NPCManager handles extreme game state values', (t) => {
  ...
  extremeGameState.reputation.locations['Test Location'] =
...
test('NPCManager handles non-existent locations and NPCs', (t) => {
  ...
  const encounter = npcManager.rollForEncounter(
    'NonExistentLocation',
    gameState
  )
  t.is(encounter, undefined, 'Should return undefined when no NPCs available')
  ...
  for (const value of extremeValues) {
    t.notThrows(() => {
      ...
      t.true(
        ...
        `Should return number for value ${value}`
      )
      t.true(
        modifier > 0,
        `Price modifier should be positive for value ${value}`
      )
...
test('ReputationManager handles invalid reputation changes', (t) => {
  ...
  t.notThrows(() => {
    const result = ReputationManager.applyReputationChange(
      gameState,
      extremeChange
    )
    t.true(
      typeof result.reputation.global === 'number',
      'Should clamp extreme values'
    )
    t.true(
      result.reputation.global >= -100 && result.reputation.global <= 100,
...
test('EnhancedEconomyManager handles invalid market data', (t) => {
  ...
  t.notThrows(() => {
    const price =
      EnhancedEconomyManager.calculateDynamicPrice(invalidMarketData)
    t.true(
      typeof price === 'number',
      'Should return number even with invalid data'
    )
...
test('EnhancedEconomyManager handles extreme market conditions', (t) => {
  ...
  t.notThrows(() => {
    const price =
      EnhancedEconomyManager.calculateDynamicPrice(extremeMarketData)
...
test('AnimationManager handles invalid animation data', async (t) => {
  ...
  for (const frames of invalidFrames) {
    ...
    t.false(
      validation.isValid,
      `Should detect invalid frames: ${JSON.stringify(frames)}`
    )
  ...
  t.notThrows(() => {
    const animation = animationManager.getNPCAnimation(
      'non_existent_npc',
      'idle'
    )
    t.true(
      animation.length > 0,
      'Should return default animation for non-existent NPC'
    )
  ...
  t.notThrows(() => {
    ...
    t.true(
      animation.length > 0,
      'Should return default animation for invalid type'
    )
...
test('AnimationManager handles memory pressure and cleanup', async (t) => {
  ...
  for (let i = 0; i < 1000; i++) {
    ...
    const largeFrames = Array.from({ length: 100 }, () => [
      'large',
      'frame',
      'data',
    ])
  ...
  t.true(
    ...
    'Should limit cache size under memory pressure'
  )
...
test('NPCTrading handles invalid trade scenarios', (t) => {
  ...
  invalidTradeNPC.trades = [
    {
      ...
      price: -100,
      quantity: 0,
      reputationRequirement: Number.NaN,
    },
    null as any,
    undefined as any,
  ]
  ...
  t.notThrows(() => {
    ...
    t.true(
      ...
      'Should return array even with invalid trades'
    )
      ...
      Promise.resolve().then(() => {
        try {
          ...
          if (i % 3 === 0) {
            ...
            ReputationManager.calculatePriceModifier(Number.NaN)
...
test('Resource cleanup and memory management under error conditions', async (t) => {
  ...
  t.true(
    ...
    'Should limit memory usage even with error conditions'
  )
