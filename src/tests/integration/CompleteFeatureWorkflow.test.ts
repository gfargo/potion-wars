  ...
  information: [
    {
      id: 'market_tip',
      content:
        'I hear the Royal Castle is paying premium for healing potions this week.',
      category: 'market',
...
test('Complete NPC encounter workflow with dialogue and trading', async (t) => {
  ...
  const tradeChoice = initialNode.choices.find((choice: any) =>
    choice.text.includes('wares')
  )
  ...
  updatedState = DialogueEngine.handleChoice(
    basicTradeChoice!,
    updatedState,
    npc.location
  )
  ...
  const finalState = DialogueEngine.handleChoice(
    purchaseChoice!,
    updatedState,
    npc.location
  )
  ...
  t.true(
    finalState.cash < gameState.cash,
    'Cash should be reduced after purchase'
  )
  t.true(
    finalState.inventory['Mana Potion']! > gameState.inventory['Mana Potion']!,
  ...
  t.true(
    finalReputation >= initialReputation,
    'Reputation should improve or stay same after successful trade'
  )
})
...
test('Market dynamics integration with reputation and NPC trading', (t) => {
  ...
  t.true(
    Math.abs(dynamicPrice - initialPrice * reputationModifier) < initialPrice,
  ...
  t.true(
    updatedMarketData.demand > initialMarketData.demand,
    'Demand should increase after purchase'
  )
  t.true(
    updatedMarketData.supply < initialMarketData.supply,
    'Supply should decrease after purchase'
  )
  t.is(
    updatedMarketData.history.length,
    initialMarketData.history.length + 1,
  ...
  const highRepTrade = tradeOffers.find(
    (trade: any) => trade.reputationRequirement > 0
  )
  if (highRepTrade) {
    const hasAccess = playerReputation >= highRepTrade.reputationRequired
    t.is(
      tradeOffers.includes(highRepTrade),
      hasAccess,
...
test('Animation system integration with NPC encounters', async (t) => {
  ...
  t.true(
    time1 < 50,
    `10 cached animation calls should complete in under 50ms, took ${time1}ms`
  )
  ...
  t.true(
    travelAnimation.frames.length > 0,
    'Travel animation should have frames'
  )

  // Step 6: Test encounter animations
  const encounterAnimation = animationManager.getEncounterAnimation('combat')
  t.true(
    encounterAnimation.length > 0,
    'Should have encounter animation frames'
  )
})
...
test('Save/load compatibility with all new features', (t) => {
  ...
  const updatedState = ReputationManager.applyReputationChange(
    gameState,
    reputationChange
  )

  // Step 3: Update market data
  const marketUpdatedState =
    EnhancedEconomyManager.updateMarketDynamics(updatedState)
  ...
  t.deepEqual(
    ...
    marketUpdatedState.reputation,
    'Reputation data should be preserved'
  )
  t.deepEqual(
    deserializedState.marketData,
    marketUpdatedState.marketData,
    'Market data should be preserved'
  )
  t.deepEqual(
    deserializedState.inventory,
    marketUpdatedState.inventory,
  ...
  t.true(
    loadedReputation > 0,
    'Reputation system should work with loaded state'
  )
...
test('Error handling and edge cases across all systems', async (t) => {
  ...
  t.throws(
    ...
    { instanceOf: Error },
    'Should throw error for invalid NPC data'
  )
  ...
  t.is(
    nonExistentNPC,
    undefined,
    'Should return undefined for non-existent NPC'
  )

  // Test 3: Invalid reputation values
  const extremeReputation = ReputationManager.calculatePriceModifier(999_999)
  t.true(
    extremeReputation > 0,
    'Should handle extreme reputation values gracefully'
  )
  ...
  t.true(
    ...
    'Should return number even with extreme values'
  )
})
...
test('Performance with complex multi-system interactions', async (t) => {
  ...
  t.true(
    time < 500,
    `50 complex multi-system operations took ${time}ms, should be under 500ms`
  )
})
...
test('Memory usage with extended gameplay simulation', async (t) => {
  ...
  t.true(
    ...
    'Animation cache should be limited after extended use'
  )
