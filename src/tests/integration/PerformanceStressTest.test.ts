...
const generateRandomNPC = (id: string, locationIndex: number): NPC => ({
  id,
  name: `NPC ${id}`,
  type: ['merchant', 'informant', 'guard', 'citizen'][
...
const generateRandomGameState = (day: number): GameState => ({
  day,
  location: {
    name: `Location_${Math.floor(
      Math.random() * STRESS_TEST_CONFIG.LOCATION_COUNT
    )}`,
...
test('NPC system performance under heavy load', async (t) => {
  ...
  t.true(
    ...
    `NPC registration of ${STRESS_TEST_CONFIG.NPC_COUNT} NPCs took ${registrationTime}ms, should be under 1000ms`
  )
  ...
  for (let i = 0; i < STRESS_TEST_CONFIG.OPERATION_COUNT; i++) {
    ...
    t.true(
      npcs.length <=
        STRESS_TEST_CONFIG.NPC_COUNT / STRESS_TEST_CONFIG.LOCATION_COUNT + 10,
      'Should not return excessive NPCs'
    )
  }
  ...
  t.true(
    queryTime < STRESS_TEST_CONFIG.TIME_LIMIT_MS,
    `${STRESS_TEST_CONFIG.OPERATION_COUNT} NPC queries took ${queryTime}ms, should be under ${STRESS_TEST_CONFIG.TIME_LIMIT_MS}ms`
  )
  ...
  t.true(
    encounterTime < 500,
    `1000 encounter rolls took ${encounterTime}ms, should be under 500ms`
  )
...
test('Reputation system performance with complex calculations', (t) => {
  ...
  t.true(
    priceModifierTime < 1000,
    `${STRESS_TEST_CONFIG.OPERATION_COUNT} price modifier calculations took ${priceModifierTime}ms, should be under 1000ms`
  )
  ...
  for (let i = 0; i < STRESS_TEST_CONFIG.OPERATION_COUNT; i++) {
    const location = `Location_${i % STRESS_TEST_CONFIG.LOCATION_COUNT}`
    const reputation = ReputationManager.getLocationReputation(
  ...
  t.true(
    locationRepTime < 1000,
    `${STRESS_TEST_CONFIG.OPERATION_COUNT} location reputation calculations took ${locationRepTime}ms, should be under 1000ms`
  )
  ...
  for (let i = 0; i < STRESS_TEST_CONFIG.OPERATION_COUNT; i++) {
    ...
    const location = `Location_${i % STRESS_TEST_CONFIG.LOCATION_COUNT}`
    const reputation = ReputationManager.getNPCReputation(
      gameState.reputation,
      npcId,
      location
    )
  ...
  t.true(
    npcRepTime < 1000,
    `${STRESS_TEST_CONFIG.OPERATION_COUNT} NPC reputation calculations took ${npcRepTime}ms, should be under 1000ms`
  )
})
...
test('Market system performance with complex market dynamics', (t) => {
  ...
  const priceCalculateStart = performance.now()
  for (let i = 0; i < STRESS_TEST_CONFIG.OPERATION_COUNT; i++) {
    const location = Object.keys(gameState.marketData)[
      i % Object.keys(gameState.marketData).length
    ]!
    ...
    const price = EnhancedEconomyManager.calculateDynamicPrice(
      marketData,
      reputationModifier
    )
  ...
  const priceCalculateTime = performance.now() - priceCalculateStart

  t.true(
    priceCalculateTime < 2000,
    `${STRESS_TEST_CONFIG.OPERATION_COUNT} price calculations took ${priceCalculateTime}ms, should be under 2000ms`
  )

  // Step 2: Test market trend calculations
  const trendCalculateStart = performance.now()
  for (let i = 0; i < 1000; i++) {
    const location = Object.keys(gameState.marketData)[
      i % Object.keys(gameState.marketData).length
    ]!
    ...
    t.true(
      ['rising', 'falling', 'stable', 'volatile'].includes(trend),
      'Should return valid trend'
    )
  }

  const trendCalculateTime = performance.now() - trendCalculateStart

  t.true(
    trendCalculateTime < 1000,
    `1000 trend calculations took ${trendCalculateTime}ms, should be under 1000ms`
  )
  ...
  t.true(
    updateTime < 2000,
    `100 market updates took ${updateTime}ms, should be under 2000ms`
  )
})
...
test('Animation system performance and memory management', async (t) => {
  ...
  for (let i = 0; i < STRESS_TEST_CONFIG.OPERATION_COUNT; i++) {
    const npcId = `npc_${i % 100}`
    const animationType = ['idle', 'talking', 'trading'][i % 3] as
      | 'idle'
      | 'talking'
      | 'trading'
  ...
  t.true(
    retrievalTime < 1000,
    `${STRESS_TEST_CONFIG.OPERATION_COUNT} animation retrievals took ${retrievalTime}ms, should be under 1000ms`
  )
  ...
  t.true(
    travelTime < 500,
    `1000 travel animation retrievals took ${travelTime}ms, should be under 500ms`
  )
...
test('Concurrent operations performance', async (t) => {
  ...
  t.is(
    results.length,
    STRESS_TEST_CONFIG.CONCURRENT_OPERATIONS,
    'All concurrent operations should complete'
  )
  t.true(
    concurrentTime < 3000,
    `${STRESS_TEST_CONFIG.CONCURRENT_OPERATIONS} concurrent operations took ${concurrentTime}ms, should be under 3000ms`
  )
})
...
test('Memory usage under extended simulation', async (t) => {
  ...
  for (let day = 1; day <= 100; day++) {
    ...
    gameState = ReputationManager.applyReputationChange(
      gameState,
      reputationChange
    )
  }
  ...
  // Verify performance
  t.true(
    simulationTime < 10_000,
    `100-day simulation took ${simulationTime}ms, should be under 10000ms`
  )

  // Verify memory management
  const animationStats = animationManager.getMemoryStats()
  t.true(
    animationStats.totalCacheSize < 200,
    `Animation cache size ${animationStats.totalCacheSize} should be reasonable after extended simulation`
  )
  ...
  t.true(
    ...
    'Reputation system should still work after extended simulation'
  )
})
...
test('Error handling under stress conditions', async (t) => {
  ...
  extremeGameState.reputation.global = 999_999
  extremeGameState.cash = -999_999
  ...
  t.notThrows(() => {
    for (let i = 0; i < 1000; i++) {
      ...
      ReputationManager.calculatePriceModifier(999_999)
      ReputationManager.calculatePriceModifier(-999_999)
